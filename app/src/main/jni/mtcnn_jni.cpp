...
#include "face/face/face_engine.h"
...
Java_com_mtcnn_1as_MTCNN_FaceDetectionModelInit(JNIEnv *env, jobject instance,
    ...
    face_engine = new FaceEngine();
    face_engine->LoadModel(tFaceModelDir.data());
...
JNIEXPORT jboolean JNICALL
Java_com_mtcnn_1as_MTCNN_FaceCompare(JNIEnv *env, jobject instance, jobject obj_bitmap,jobject obj_compare) {
    ...
    void * comparePixels;                                            // Save picture pixel data
    AndroidBitmapInfo compareInfo;                                   // Save picture parameters
    ...
    ASSERT_FALSE( AndroidBitmap_getInfo(env, obj_compare, &compareInfo) >= 0);        // Get picture parameters
    ASSERT_FALSE( compareInfo.format == ANDROID_BITMAP_FORMAT_RGBA_8888
                  || compareInfo.format == ANDROID_BITMAP_FORMAT_RGB_565 );          // Only ARGB? 8888 and RGB? 565 are supported
    ASSERT_FALSE( AndroidBitmap_lockPixels(env, obj_compare, &comparePixels) >= 0 );  // Get picture pixels (lock memory block)
    ASSERT_FALSE( comparePixels );
    ...
    cv::Mat compareTmp(compareInfo.height, compareInfo.width, CV_8UC4, comparePixels);    // Establish temporary mat



    //convert RGB to BGR
    cv::cvtColor(tmp,tmp,cv::COLOR_RGBA2BGR);
    cv::cvtColor(compareTmp,compareTmp,cv::COLOR_RGBA2BGR);

    double start = static_cast<double>(cv::getTickCount());
    std::vector<FaceInfo> faces;
    std::vector<FaceInfo> comparefaces;
    face_engine->DetectFace(tmp, &faces);
    face_engine->DetectFace(compareTmp, &comparefaces);



    cv::Rect face = faces.at(0).location_;
    std::vector<cv::Point2f> keypoints;
    face_engine->ExtractKeypoints(tmp, face, &keypoints);

    cv::Mat face_aligned;
    face_engine->AlignFace(tmp, keypoints, &face_aligned);





    cv::Rect face2 = comparefaces.at(0).location_;
    std::vector<cv::Point2f> keypoints2;
    face_engine->ExtractKeypoints(compareTmp, face2, &keypoints2);

    cv::Mat face_aligned2;
    face_engine->AlignFace(compareTmp, keypoints2, &face_aligned2);
    ...
    face_engine->ExtractFeature(face_aligned, &feature1);
    face_engine->ExtractFeature(face_aligned2, &feature2);
    ...
    for (int i = 0; i < static_cast<int>(faces.size()); ++i) {
        ...
        cv::rectangle(imgInput, face, cv::Scalar(0, 255, 0), 2);
    }
